...
for one in ifile:
    if not one.startswith("#"):
        ...
        tag = ref.fetch(ones[0],int(ones[1])-25,int(ones[1])-1) + "[" +"/".join(ones[3:5])+"]"+ref.fetch(ones[0],int(ones[1]),int(ones[1])+25)
        ...
        primer = bindings.designPrimers(
            ...
            },
            {
                ...
                'PRIMER_PRODUCT_SIZE_RANGE': [[100,125],[125,150],[150,175],[200,225],
        ...
        if primer['PRIMER_PAIR_NUM_RETURNED'] >= 1:
            ofile.write("\t".join(["#"+seq_id,tag,primer['PRIMER_LEFT_0_SEQUENCE'],primer['PRIMER_RIGHT_0_SEQUENCE']]) +"\n")
            ...
            ofile.write("\t".join(["#"+seq_id,tag,primer2['PRIMER_LEFT_0_SEQUENCE'],primer2['PRIMER_RIGHT_0_SEQUENCE']]) +"\n")
            opt1 = "\t".join([seq_id+"_F",
