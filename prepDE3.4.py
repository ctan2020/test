...
for s in samples:
    ...
    try:
        with open(next(glob.iglob(os.path.join(opts.input,s,"*.gtf")))) as f:
...
for q, s in enumerate(samples):
    ...
    try:
        with open(next(glob.iglob(os.path.join(opts.input,s,"*.gtf")))) as f: #grabs first .gtf file it finds inside the sample subdirectory
